...
const show = async (req, res) => {
  const [error, result] = await to(service.show(req.params.id, req.user))
  handleResponse(error, result, req, res)
}
...
const destroyMultiple = async (req, res) => {
  const [error, result] = await to(service.destroyMultiple(req.body, req.user))
  handleResponse(error, result, req, res)
}

const sendNotification = async (req, res) => {
  const [error, result] = await to(service.sendNotification(req.body, req.user))
  handleResponse(error, result, req, res)
}


const markAsRead = async (req, res) => {
  const [error, result] = await to(service.markAsRead(req.body, req.user))
  handleResponse(error, result, req, res)
}
